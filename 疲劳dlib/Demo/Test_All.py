...
# 用作计数器,1000次EAR和,1000次MAR，计数1000次的EAR和MAR值，求平均作为阈值
testCount = 0
sum_EAR = 0
sum_MAR = 0
...
predictor = dlib.shape_predictor(
    '../Modul/shape_predictor_68_face_landmarks.dat')
...
while True:
    ...
    for rect in rects:
        ...
        if testCount < 500:
            ...
            sum_EAR += ear
            sum_MAR += mar
            cv2.putText(frame, "Please be patient", (20, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(frame, "It's Getting the Threshold of your Facial Feature", (20, 300),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(frame, "CountDown:{}".format(500-testCount), (20, 400),cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)  # 展示倒计时
            continue
        elif testCount == 500:
            testCount += 1
            EYE_AR_THRESH = sum_EAR / 500
            MAR_THRESH = sum_MAR / 500

            print('眼睛长宽比100次取平均的阈值:{:.2f} '.format(EYE_AR_THRESH))
            print('嘴巴长宽比100次取平均的阈值:{:.2f} '.format(MAR_THRESH))
