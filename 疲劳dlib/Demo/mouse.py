...
# 用作计数器,100次EAR和,100次MAR，先计数50次的ear值，求平均作为阈值
testCount = 0
sum_EAR = 0
sum_MAR = 0
...
while True:
    ...
    for rect in rects:
        ...
        if testCount < 100:
            ...
            cv2.putText(frame, "Now,It is Getting the Threshold of your Facial Feature", (20, 300),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            continue
        elif testCount == 100:
            testCount += 1
            EYE_AR_THRESH = sum_EAR / 100
            MAR_THRESH = sum_MAR / 100
            ...
            '''
                计算张嘴评分，如果大于阈值，则加1，如果连续3次都大于阈值，则表示打了一次哈欠，同一次哈欠大约在3帧
            '''
            # 同理，判断是否打哈欠
